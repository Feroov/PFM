...
const translations = {
    en: {
        ...
        expensesBreakdown: "Expenses Breakdown by Category",
        ...
        noTransactions: "No transactions available to export.",
        ok: "OK",
    ...
    tr: {
        ...
        cancelDelete: "İptal",
        errorMessage: "Lütfen geçerli bir açıklama ve miktar girin.",
        dataManagement: "Veri Yönetimi",
        incomeVsExpenses: "Gelir ve Giderler",
        expensesBreakdown: "Kategorilere Göre Giderler",
        noData: "Veri bulunamadı.",
        recurringInterval: "Yinelenen Aralık",
        daily: "Günlük",
        weekly: "Haftalık",
        monthly: "Aylık",
        exportFailed: "Dışa Aktarma Başarısız",
        noTransactions: "Dışa aktarmak için işlem yok.",
        ok: "Tamam",
    ...
    de: {
        ...
        income: "Einkommen",
        expenses: "Ausgaben",
        addTransaction: "Neue Transaktion hinzufügen",
        ...
        errorMessage: "Bitte geben Sie eine gültige Beschreibung und einen gültigen Betrag ein.",
        dataManagement: "Datenverwaltung",
        incomeVsExpenses: "Einkommen vs Ausgaben",
        expensesBreakdown: "Aufschlüsselung der Ausgaben nach Kategorie",
        noData: "Keine Daten gefunden.",
        recurringInterval: "Wiederkehrendes Intervall",
        daily: "Täglich",
        weekly: "Wöchentlich",
        monthly: "Monatlich",
        exportFailed: "Export fehlgeschlagen",
        noTransactions: "Keine Transaktionen zum Exportieren verfügbar.",
        ok: "OK",
    ...
    es: {
        ...
        errorMessage: "Por favor ingrese una descripción y cantidad válidas.",
        ...
        noTransactions: "No hay transacciones disponibles para exportar.",
        ok: "OK",
...
function translatePage(language) {
    const elementsToTranslate = {
        balanceEl: { type: 'textContent', key: 'balance' },
        incomeTotalEl: { type: 'textContent', key: 'income' },
        expenseTotalEl: { type: 'textContent', key: 'expenses' },
        addTransactionBtn: { type: 'textContent', key: 'addTransaction' },
        descriptionElLabel: { type: 'textContent', key: 'description' },
        amountElLabel: { type: 'textContent', key: 'amount' },
        transactionTypeElLabel: { type: 'textContent', key: 'type' },
        categoryElLabel: { type: 'textContent', key: 'category' },
        recurringCheckboxLabel: { type: 'textContent', key: 'recurring' },
        exportCsvBtn: { type: 'textContent', key: 'csv' },
        exportExcelBtn: { type: 'textContent', key: 'excel' },
        exportPdfBtn: { type: 'textContent', key: 'pdf' },
        exportJsonBtn: { type: 'textContent', key: 'json' },
        deleteAllBtn: { type: 'textContent', key: 'deleteAll' },
        descriptionEl: { type: 'placeholder', key: 'description' }, // Input placeholder
        amountEl: { type: 'placeholder', key: 'amount' }, // Input placeholder
        transactionHistoryTitle: { type: 'textContent', key: 'editTransaction' }, // Section title
        exportData: { type: 'textContent', key: 'exportData' }, // Section title
        importData: { type: 'textContent', key: 'importData' }, // Section title
        incomeVsExpensesTitle: { type: 'textContent', key: 'incomeVsExpenses' }, // Chart title
        expensesBreakdownTitle: { type: 'textContent', key: 'expensesBreakdown' }, // Chart title
        recurringInterval: { type: 'textContent', key: 'recurringInterval' }, // Recurring interval label
        daily: { type: 'textContent', key: 'daily' }, // Daily option
        weekly: { type: 'textContent', key: 'weekly' }, // Weekly option
        monthly: { type: 'textContent', key: 'monthly' } // Monthly option
    };

    // Apply the translation to each DOM element
    Object.keys(elementsToTranslate).forEach(elementId => {
        const { type, key } = elementsToTranslate[elementId];
    ...
    document.getElementById('transaction-type').options[0].text = translations[language]['income'];
    document.getElementById('transaction-type').options[1].text = translations[language]['expenses'];
    
    // Translate recurring interval options
    document.getElementById('recurring-interval').options[0].text = translations[language]['daily'];
    document.getElementById('recurring-interval').options[1].text = translations[language]['weekly'];
    document.getElementById('recurring-interval').options[2].text = translations[language]['monthly'];

    // Translate category options for income and expense categories
    const incomeCategories = {
        en: ['Salary', 'Bonus', 'Freelance', 'Other'],
        tr: ['Maaş', 'İkramiye', 'Serbest Çalışma', 'Diğer'],
        de: ['Gehalt', 'Bonus', 'Freiberuflich', 'Andere'],
        es: ['Salario', 'Bono', 'Freelance', 'Otro']
    };

    const expenseCategories = {
        en: ['Rent', 'Food', 'Utilities', 'Entertainment', 'Other'],
        tr: ['Kira', 'Yemek', 'Faturalar', 'Eğlence', 'Diğer'],
        de: ['Miete', 'Essen', 'Nebenkosten', 'Unterhaltung', 'Andere'],
        es: ['Renta', 'Comida', 'Servicios', 'Entretenimiento', 'Otro']
    };

    // Update the categories in the transaction form based on the selected language
    const selectedType = document.getElementById('transaction-type').value;
    const categoryEl = document.getElementById('category');
    categoryEl.innerHTML = ''; // Clear current categories

    const categories = selectedType === 'income' ? incomeCategories[language] : expenseCategories[language];
    ...
    // Update filter by category options
    const categoryFilterEl = document.getElementById('category-filter');
    categoryFilterEl.innerHTML = ''; // Clear current filter options

    // Add "All Categories" as the first option
    const allOption = document.createElement('option');
    allOption.value = 'all';
    allOption.textContent = language === 'tr' ? 'Tüm Kategoriler' : 'All Categories'; // Example for Turkish
    categoryFilterEl.appendChild(allOption);

    // Add filtered categories for the expense categories
    expenseCategories[language].forEach(category => {
        const option = document.createElement('option');
        option.value = category;
        option.textContent = category;
    ...
    // Transaction history sample items translation
    const transactionHistoryItems = {
        ...
        tr: {
            ...
            streaming: 'Yayın Aboneliği'
        },
        de: {
            dinner: 'Abendessen mit Freunden',
            streaming: 'Streaming-Abonnement'
        },
        es: {
            dinner: 'Cena con amigos',
            streaming: 'Suscripción de streaming'
        }
    };

    const transactionDescriptions = document.querySelectorAll('.transaction-description');
    transactionDescriptions.forEach(descEl => {
        const originalText = descEl.textContent.trim().toLowerCase();

        if (originalText.includes('dinner')) {
            descEl.textContent = transactionHistoryItems[language]['dinner'];
        } else if (originalText.includes('streaming')) {
            descEl.textContent = transactionHistoryItems[language]['streaming'];
        }
    });
}
...
document.getElementById('language-selector').addEventListener('change', function (e) {
    ...
    localStorage.setItem('language', selectedLanguage);  // Save the language in localStorage
    translatePage(selectedLanguage);  // Call the translate function
});
